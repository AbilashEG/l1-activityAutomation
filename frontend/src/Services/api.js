// Services/api.js - PRODUCTION-READY VERSION WITH ASSESSMENT REPORT
...
// ✅ Assessment Report Generation API
export const generateAssessmentReport = async (params) => {
  try {
    if (!params.accountId || !params.region)
      throw new Error('Missing required fields: accountId, region');
    if (params.templateFile || params.imageFile) {
      // If backend expects file uploads
      const formData = new FormData();
      formData.append('accountId', params.accountId);
      formData.append('region', params.region);
      if (params.templateFile) formData.append('template', params.templateFile);
      if (params.imageFile) formData.append('image', params.imageFile);
      const response = await fetch(`${BASE}/api/generate-assessment-report`, {
        method: 'POST',
        body: formData,
      });
      const contentType = response.headers.get('content-type');
      ...
      if (response.ok && result.success) {
        return {
          ...
          data: {
            ...
            reportBase64: result.reportBase64,
      ...
      // If backend only expects JSON
      return await apiCall('/api/generate-assessment-report', {
        method: 'POST',
        body: JSON.stringify({
    ...
    try {
      if (contentType && contentType.includes('application/json')) {
        ...
        if (response.ok || response.status === 200) {
          result = { success: true, message: 'Alarms configured successfully' };
        } else {
...
export const convertGp2ToGp3Volumes = async (params) => {
  try {
    ...
    if (volumeIds.length === 0) throw new Error('No volumes specified for conversion');
    const results = [];
    let successCount = 0,
      failedCount = 0;
